...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-background" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <aside className={`fixed ${language === 'ar' ? 'right-0' : 'left-0'} top-0 h-full w-64 bg-card border-${language === 'ar' ? 'l' : 'r'} border-border z-10 backdrop-blur-xl flex flex-col`}>
        <div className="p-6 flex-1 overflow-y-auto pb-4">
          {/* Logo Section - Elite Style */}
          <div className="flex items-center gap-3 mb-8">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center relative overflow-hidden bg-muted group cursor-pointer">
            ...
            {menuItems.map((item) => {
              ...
              return (
                <Button
                  ...
                  className={`w-full justify-start transition-all duration-300 gap-3 group ${
                    isActive 
        ...
        <div className="p-6 border-t border-border bg-card/80 backdrop-blur-sm shrink-0">
          <div className="flex items-center gap-3 mb-3 p-3 rounded-lg bg-accent/50">
          ...
          <Button
            variant="outline"
            size="sm"
            className="w-full hover:bg-destructive hover:text-destructive-foreground transition-all duration-200 group"
            onClick={handleLogout}
          >
            <LogOut className={`${language === 'ar' ? 'mr-2' : 'ml-2'} h-4 w-4 transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-12`} />
            {t('sign_out')}
          </Button>
        </div>
      </aside>
      ...
      {/* Main Content */}
      <main className={`${language === 'ar' ? 'mr-64' : 'ml-64'} min-h-screen`}>
